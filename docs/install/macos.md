...
Two models are needed — the chat model and the embedding model used by the
RAG indexer. Pull them before indexing:
...
**Apple Silicon note**: Ollama detects Metal automatically. No extra config
needed — inference will already be using GPU acceleration.
...
The Docker images are built as multi-arch manifests (`linux/amd64` + `linux/arm64`).
Docker Desktop on Apple Silicon runs the native arm64 images — no Rosetta
emulation for any service, including Kiwix and Open WebUI.
...
directory (avoids `/var/lib/` permission issues), saves paths to
`~/.config/allarkive/config.json` for future runs, detects port conflicts
and auto-assigns alternatives, and pulls both the chat model and embedding
model before running the indexer. The manual steps above are the exact
equivalent.
